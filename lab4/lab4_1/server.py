...
def handle(client):
    try:
        while SERVER_WORKING:
            ...
            if "login" not in clients[client]:
                if typeOp == TYPES["CLT"]["REG"]:  # Регистрация
                    regRequest(client, packet)
                    textOp = "REG"
                elif typeOp == TYPES["CLT"]["AUT"]:  # Авторизация
                    authRequest(client, packet)
                    textOp = "AUT"
            else:
                if typeOp == TYPES["CLT"]["LST"]:  # Список тестов
                    sendList(client)
                    textOp = "LST"
                elif typeOp == TYPES["CLT"]["RES"]:  # Результат последнего теста
                    resultRequest(client)
                    textOp = "RES"
                elif typeOp == TYPES["CLT"]["TST"]:  # Получить тест
                    testRequest(client, packet)
                    textOp = "TST"
                elif typeOp == TYPES["CLT"]["ANS"]:  # Ответ на вопрос
                    answerResponse(client, packet)
                    textOp = "ANS"
                elif typeOp == TYPES["CLT"]["DSC"]:  # Отключиться от сервера
...
def startServer():
    # Создаём список с названиями тестов и их UID
    createList()
